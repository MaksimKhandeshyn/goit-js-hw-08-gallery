...
function makeItems() {
  galleryItems.map((el) => {
    ...
    imgTag.setAttribute("src", el.preview);
    imgTag.setAttribute("data-source", el.original);
    // linkTag.setAttribute("href", el.original);

    //Добавление елементов
    linkTag.append(imgTag);
    galleryLi.append(linkTag);
    refs.galleryList.append(galleryLi);

    galleryLi.addEventListener("click", () => {
      refs.lightbox.classList.add("is-open");
      refs.image.setAttribute("src", el.original);
    });

    refs.button.addEventListener("click", () => {
      refs.lightbox.classList.remove("is-open");
      refs.image.setAttribute("src", "");
    });
  });
}
makeItems();
